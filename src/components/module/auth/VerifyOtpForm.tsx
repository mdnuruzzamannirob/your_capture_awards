...
import { useForgotPasswordMutation, useVerifyOTPMutation } from '@/store/apis/authApi';
...
export default function VerifyOtpForm() {
  ...
  return (
    <div className="mx-auto w-full max-w-lg space-y-16 px-5 pt-5 pb-10">
      ...
      <div className="border-primary/40 w-full space-y-5 rounded-md border p-8 md:p-10">
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="flex w-full items-center gap-3">
              {otp.map((digit, idx) => (
                <input
                  ...
                  onPaste={handlePaste}
                  className="border-border-strong focus:border-primary size-8 rounded border bg-transparent text-center outline-none md:size-10"
                />
              ))}
            </div>
          </div>

          <p className="text-caption-foreground mb-4 text-xs">
            Didn&apos;t get a code?&nbsp;
            <button
              type="button"
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="bg-primary hover:bg-primary/90 disabled:hover:bg-primary text-primary-foreground mt-8 w-full rounded-sm py-2.25 transition-colors disabled:cursor-not-allowed disabled:opacity-50"
          >
