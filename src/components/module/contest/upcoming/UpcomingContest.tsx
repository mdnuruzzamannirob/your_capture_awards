...
const UpcomingContest = ({
  ...
  const { data, isLoading, refetch, isFetching, isError, error } = isAuthenticated
    ? privateQuery
    : publicQuery;
  const upcomingResult = (data as any)?.data ?? EMPTY_CONTESTS;
  ...
  useEffect(() => {
    ...
    setAllContests((prev) => {
      ...
      return [...prev, ...upcomingResult.filter((contest: any) => !existingIds.has(contest.id))];
    });
  }, [isAuthenticated, upcomingResult, page]);
